...
class PostgreSQLSchemaProvider:
    ...
    async def create_pool(self) -> None:
        ...
        if self.connection_pool is None:
            try:
                # Extract server_settings from config to avoid conflicts
                config_copy = dict(self.postgres_config)
                existing_server_settings = config_copy.pop("server_settings", {})
                
                # Merge existing server settings with our defaults
    ...
    async def ensure_schemas_loaded(self, schema_name: str, rls_user_id: str) -> None:
        ...
        if self.all_schemas is None:
            self.all_schemas = await self.get_all_schemas(
                schema_name, rls_user_id=rls_user_id
            )
    ...
    async def get_table_schema(
        self, table_name: str, rls_user_id: str
    ) -> Dict[str, Any]:
                    ...
                    try:
                        if key == "price_range":
                            ...
                            if result and result["min_price"] is not None:
                                enum_data[key] = (
                                    f"${result['min_price']:.2f} - ${result['max_price']:.2f}"
                                )
                            ...
                            enum_data[key] = await self.fetch_distinct_values(
                ...
                "foreign_keys": [
                    {
                        ...
                        "relationship_type": self.infer_relationship_type(
                            f"{schema_name}.{fk['foreign_table_name']}"
                        ),
                    }
    ...
    async def get_all_schemas(
        self, schema_name: str, rls_user_id: str
        ...
        for table_name in table_names:
            ...
            schema_data = await self.get_table_schema(
                qualified_name, rls_user_id=rls_user_id
            )
    ...
    def format_schema_metadata_for_ai(self, schema: Dict[str, Any]) -> str:
        ...
        try:
            _, table_name_only = (
                self._parse_table_name(table_display)
                if table_display
                else ("", "unknown")
            )
            ...
            table_description = (
                table_display.replace("_", " ") if table_display else "unknown"
            )
    ...
    async def get_table_metadata_from_list(
        self, table_names: List[str], rls_user_id: str
    ) -> str:
    ...
    async def execute_query(self, sql_query: str, rls_user_id: str) -> str:
        ...
        try:
            ...
            return json.dumps(
                {
                    "err": f"PostgreSQL query failed: {e!s}",
                    "q": sql_query,
                    "c": [],
                    "r": [],
                    "n": 0,
...
async def main() -> None:
    ...
    if not await test_connection():
        logger.error("❌ Error: Cannot connect to PostgreSQL using host: %s:%s", 
                    POSTGRES_CONNECTION_PARAMS.get("host", "unknown"),
                    POSTGRES_CONNECTION_PARAMS.get("port", "unknown"))
        logger.error("   Please verify:")
        logger.error("   1. PostgreSQL is running")
        logger.error("   2. Database '%s' exists", POSTGRES_CONNECTION_PARAMS.get("database", "unknown"))
        logger.error("   3. PostgreSQL environment variables are correct")
    ...
    try:
        async with PostgreSQLSchemaProvider() as provider:
            ...
            logger.info(
                ...
                await provider.get_table_metadata_from_list(
                    all_table_names, rls_user_id=MANAGER_ID
                ),
            )
